...
exports.create = async (req, res) => {
  ...
  let studentInfo = await StudentInfo.findAll({where: {userId: session.userId}})
  console.log(studentInfo[0].dataValues.id)
  ...
  const FlightPlanData = {
    id: req.body.id,
    studentInfoId: studentInfo[0].dataValues.id,
    semestersToGrad: semestersToGrad
  };

  let existingFlightPlan = await FlightPlan.findOne({where: {studentInfoId: studentInfo[0].dataValues.id, semestersToGrad: semestersToGrad}})
  if (existingFlightPlan != null) {res.status(200); res.send({message: "FlightPlan found, not creating"})}
    else {
    ...
    FlightPlan.create(FlightPlanData)
      ...
      let studentInfoMajors = await StudentInfoMajor.findAll({where: {studentInfoId: studentInfo[0].dataValues.id}})
